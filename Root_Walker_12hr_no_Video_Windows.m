...
thresh_val  = 0.52;
%Tune the size of small objects
rm_small = 8000;
%Tune the size of big objects
rm_large = 150000;
...
for n = 1:length(child_3_sub)
    ...
    to_remove = [];
    for i = 1:root_num
        if (sum(diff(cell2mat(all_root_dist(:,i))) > 45) > 0)
            %checks if there is a jump greater than 45
            %save i value for root to remove
            to_remove = [to_remove i];
            fprintf('JUMP: %d\n', i);
        elseif (nnz(~diff(cell2mat(all_root_dist(:,i)))) > 45)
            %checks if there are more than 45 timesteps where the length
