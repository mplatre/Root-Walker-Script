...
for n = 1:length(child_2_sub)
    ...
    to_remove = [];
    for i = 1:root_num
        if (sum(diff(cell2mat(all_root_dist(:,i))) > 10) > 0)
            %checks if there is a jump greater than 10
            %save i value for root to remove
            to_remove = [to_remove i];
            fprintf('JUMP: %d\n', i);
        elseif (nnz(~diff(cell2mat(all_root_dist(:,i)))) > 40)
            %checks if there are more than 40 timesteps where the length
...
for h = 1:length(xfold_t_step_proc)
    ...
    v = VideoWriter(char(output_name));
    open(v)
    writeVideo(v,F)
    close(v)
